...
Run `azycode` without arguments to open the interactive terminal workspace. Type `/` to show the command palette, type a task directly, or use slash commands such as `/status`, `/health`, `/doctor`, `/dashboard`, `/sessions`, `/session`, `/tools`, `/policy`, `/goals`, `/goal`, `/missions`, `/mission`, `/memory`, `/agents`, `/agent`, `/providers`, `/provider`, `/credentials`, `/keys`, `/login`, `/new`, `/compact`, `/mode`, `/reasoning`, `/model`, `/models`, `/profile`, `/context`, `/progress`, `/review`, `/clear`, and `/exit`.
